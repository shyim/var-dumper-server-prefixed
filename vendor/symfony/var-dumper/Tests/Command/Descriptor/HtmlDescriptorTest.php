...
class HtmlDescriptorTest extends \_PhpScoper5d36eb080763e\PHPUnit\Framework\TestCase
{
    ...
    public function testItOutputsStylesAndScriptsOnFirstDescribeCall()
    {
        ...
        $dumper = $this->createMock(\_PhpScoper5d36eb080763e\Symfony\Component\VarDumper\Dumper\HtmlDumper::class);
        $dumper->method('dump')->willReturn('[DUMPED]');
        $descriptor = new \_PhpScoper5d36eb080763e\Symfony\Component\VarDumper\Command\Descriptor\HtmlDescriptor($dumper);
        $descriptor->describe($output, new \_PhpScoper5d36eb080763e\Symfony\Component\VarDumper\Cloner\Data([[123]]), ['timestamp' => 1544804268.3668], 1);
    ...
    public function testDescribe(array $context, string $expectedOutput)
    {
        ...
        $this->assertStringMatchesFormat(\trim($expectedOutput), \trim(\preg_replace('@<style>.*</style><script>.*</script>@s', '', $output->fetch())));
    }
